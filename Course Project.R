setwd("/Users/Benny/Documents/Diverse/Coursera Getting and Cleaning data/Course Project/")
library(data.table)
##### LOAD ALL THE FILES ######
...
X_test <- read.table(file="UCI HAR Dataset/test/X_test.txt",header=F,sep="",) #Loading  X_test data
X_test <- data.table(X_test) # Create a data.table for faster access
setnames(X_test,features)
subject_test <- read.table(file="UCI HAR Dataset/test/subject_test.txt",header=F,sep="",) #Loading subejct_test data
...
y_test <- read.table(file="UCI HAR Dataset/test/y_test.txt",header=F,sep="",) #Loading y_test data
total_X_test$Activity <- y_test #Add a column with the Activity to tht total_X_test data.frame

X_train <- read.table(file="UCI HAR Dataset/train/X_train.txt",header=F,sep="",) #Loading  X_train data
...
subject_train <- read.table(file="UCI HAR Dataset/train/subject_train.txt",header=F,sep="",) #Loading subejct_train data
total_X_train$Subject <- subject_train #add a "subject" column to the total_X_train data.frame
y_train <- read.table(file="UCI HAR Dataset/train/y_train.txt",header=F,sep="",) #Loading y_train data
...
##### add activites
activity_labels <- read.table(file="UCI HAR Dataset/activity_labels.txt",header=F,sep="",) #Loading  X_train data
colnames(activity_labels) <- c("Activity","Activity_label")
#merge(data.frame(total_df$Activity),activity_labels)
